    ...
    while True:
        if receiveTelegramMessage == True:
            ...
            if telegramText == "FOTO":
                ...
                exit()
        elif boton.is_pressed:
            x = datetime.now()
            tiempo = x.strftime("%H") + ":" + x.strftime("%M")+" del "+ x.strftime("%d") +"/" +x.strftime("%m") +"/" +x.strftime("%Y")
            print('presionado')
            bot.sendMessage(chat_id, 'TIMBRE PRESIONADO A LAS ' + tiempo)
            bot.sendMessage(chat_id, 'CAPTURANDO IMAGEN')
            camera.capture('./capture.jpg')
            bot.sendPhoto(chat_id=chat_id, photo=open('./capture.jpg', 'rb'))
